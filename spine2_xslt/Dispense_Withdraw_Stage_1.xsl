...
<!--PATTERN authorRootCheck-->
	...
	<!--RULE -->
<axsl:template match="//hl7:author[hl7:AgentPersonSDS]" priority="1000" mode="M3">

		<!--ASSERT -->
<axsl:choose><axsl:when test="count(hl7:AgentPersonSDS)=1"/><axsl:otherwise>***fault|ErrorBase1634.UNABLE_TO_PROCESS|AgentPerson Element missing or blank***<axsl:value-of select="string('&#10;')"/></axsl:otherwise></axsl:choose>

		<!--ASSERT -->
<axsl:choose><axsl:when test="count(hl7:AgentPersonSDS/hl7:agentPersonSDS)=1"/><axsl:otherwise>***fault|ErrorBase1634.UNABLE_TO_PROCESS|agentPersonSDS Element missing or blank***<axsl:value-of select="string('&#10;')"/></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M3"/></axsl:template><axsl:template match="text()" priority="-1" mode="M3"/><axsl:template match="@*|node()" priority="-2" mode="M3"><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M3"/></axsl:template>

<!--PATTERN dispenserInformationCheck-->


	<!--RULE -->
<axsl:template match="//hl7:AgentPersonSDS" priority="1000" mode="M4">
...
<axsl:if test="count(hl7:id[@root='1.2.826.0.1285.0.2.0.67']/@extension)=1">***report|agentRoleProfileCodeId|<axsl:text/><axsl:value-of select="hl7:id[@root='1.2.826.0.1285.0.2.0.67']/@extension"/><axsl:text/>***<axsl:value-of select="string('&#10;')"/></axsl:if>
		...
		<!--ASSERT -->
<axsl:choose><axsl:when test="count(hl7:agentPersonSDS/hl7:id/@extension)=1"/><axsl:otherwise>***fault|ErrorBase1634.UNABLE_TO_PROCESS|Dispenser ID missing or blank***<axsl:value-of select="string('&#10;')"/></axsl:otherwise></axsl:choose>

		<!--REPORT -->
<axsl:if test="count(hl7:agentPersonSDS/hl7:id/@extension)=1">***report|dispenserCode|<axsl:text/><axsl:value-of select="hl7:agentPersonSDS/hl7:id/@extension"/><axsl:text/>***<axsl:value-of select="string('&#10;')"/></axsl:if><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/></axsl:template><axsl:template match="text()" priority="-1" mode="M4"/><axsl:template match="@*|node()" priority="-2" mode="M4"><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/></axsl:template></axsl:stylesheet>
