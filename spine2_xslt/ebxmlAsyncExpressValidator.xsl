...
<!--PATTERN AsyncExpressHeaderChecks-->
...
<axsl:template match="SOAP:Envelope[1]/SOAP:Header[1]" priority="1000" mode="M9">
		...
		<!--ASSERT -->
<axsl:choose><axsl:when test="count(eb:MessageHeader[1]/eb:DuplicateElimination) = 0"/><axsl:otherwise>***fault|ErrorBaseTms.DUPLICATE_ELIMINATION_INCONSISTENT_WITH_CONTRACT[duplicateEliminationUnexpected]***<axsl:value-of select="string('&#10;')"/></axsl:otherwise></axsl:choose>

		<!--ASSERT -->
<axsl:choose><axsl:when test="count(eb:AckRequested) = 0"/><axsl:otherwise>***fault|ErrorBaseTms.ACK_REQUESTED_INCONSISTENT_WITH_CONTRACT[ackRequiredUnexpected]***<axsl:value-of select="string('&#10;')"/></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M9"/></axsl:template><axsl:template match="text()" priority="-1" mode="M9"/><axsl:template match="@*|node()" priority="-2" mode="M9"><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M9"/></axsl:template>
