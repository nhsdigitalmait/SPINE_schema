...
<!--PATTERN consentCheck-->
	...
	<!--RULE -->
<axsl:template match="/*/hl7:ControlActEvent[1]/hl7:subject[1]/hl7:PdsUpdateRequest[1]/hl7:subject[1]/hl7:patientRole[1]/hl7:subjectOf/hl7:consent" priority="1000" mode="M2">

		<!--REPORT -->
<axsl:if test="not(hl7:code/@code='4')">***fault|ErrorBase1742.INVALID_CONSENT_VALUE***<axsl:value-of select="string('&#10;')"/></axsl:if>

		<!--REPORT -->
<axsl:if test="(@updateMode='removed')">***fault|ErrorBase1742.CONSENT_VALUES_CANNOT_BE_REMOVED***<axsl:value-of select="string('&#10;')"/></axsl:if>

		<!--REPORT -->
<axsl:if test="count(hl7:pertinentInformation/hl7:pertinentSupplementaryComments)&gt;0 and string-length(hl7:pertinentInformation/hl7:pertinentSupplementaryComments/hl7:value)=0">***fault|ErrorBase1742.SUPPLEMENTARY_COMMENTS_VALUE_MISSING***<axsl:value-of select="string('&#10;')"/></axsl:if><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M2"/></axsl:template><axsl:template match="text()" priority="-1" mode="M2"/><axsl:template match="@*|node()" priority="-2" mode="M2"><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M2"/></axsl:template>
